...
use diamond_replay::player::{BattingStats, PlayerGameStats};
use diamond_replay::replay::GameResult;
...
fn draw_header(frame: &mut ratatui::Frame, area: Rect, current_view: View, game_name: &str) {
    let views = [View::BoxScore, View::Batting, View::Pitching];
...
fn draw_footer(frame: &mut ratatui::Frame, area: Rect) {
    let footer_spans = vec![
        ...
        Span::styled("1/2/3", HEADER_STYLE),
...
fn dump_json(result: &GameResult, game_name: &str) {
...
fn main() {
    ...
    if paths.is_empty() {
        eprintln!("Usage: diamond-replay <game.json> [--json]");
        process::exit(1);
    }
