...
/// by an explicit `base_running` event or a base-state change.
fn already_handled<S: BuildHasher>(
    base: usize,
    snapshot: &BaseState,
    current: &BaseState,
    ...
    match snapshot.get(base) {
        ...
        Some(BaseOccupant::Player(id)) => {
            explicit.contains(id.as_str()) || current.get(base) != snapshot.get(base)
        }
        Some(BaseOccupant::Anonymous) => current.get(base) != snapshot.get(base),
    }
}
...
struct Ctx<'a, S: BuildHasher, S2: BuildHasher> {
    hi: usize,
    snap: &'a BaseState,
...
fn resolve_triple(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>) {
    for b in [1, 2, 3] {
        cx.score_if_live(b);
    }
    cx.bases.set(3, Some(BaseOccupant::Anonymous));
    cx.bases.set(2, None);
    cx.bases.set(1, None);
}

fn resolve_double(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>) {
    cx.score_if_live(3);
    cx.score_if_live(2);
    cx.advance_if_live(1, 3);
    cx.bases.set(2, Some(BaseOccupant::Anonymous));
}

fn resolve_single(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>, is_fly: bool) {
    if !is_fly {
        cx.score_if_live(3);
    }
...
fn resolve_advance_out(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>) {
...
fn resolve_fielders_choice(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>) {
    cx.score_if_live(3);
    cx.advance_if_live(2, 3);
    cx.bases.set(1, Some(BaseOccupant::Anonymous));
}

fn resolve_error(cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>) {
    cx.score_if_live(3);
    cx.advance_if_live(2, 3);
    cx.advance_if_live(1, 2);
    cx.bases.set(1, Some(BaseOccupant::Anonymous));
...
fn resolve_dropped_third(
    cx: &mut Ctx<'_, impl BuildHasher, impl BuildHasher>,
